...
function writePoolManifest(outDir) {
  const files = readdirSync(outDir)
    .filter((name) => /^\d{4}-\d{2}-\d{2}\.json$/.test(name))
    .sort();
  const imports = files.map(
    (name, index) =>
      `import pool${index} from "./${name}" with { type: "json" };`,
  );
  const names = files.map((_, index) => `pool${index}`).join(", ");
  const body = [
    'import type { MorningPool } from "../pool.js";',
    ...imports,
    "",
    "/** Generated by scripts/pool.mjs for Vite and the Vercel Node function. */",
    `export const MORNING_POOLS = [${names}] as MorningPool[];`,
    "",
  ].join("\n");
